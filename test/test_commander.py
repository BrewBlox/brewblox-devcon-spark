...
@pytest.fixture
def comms_mock(mocker):
    m = mocker.patch(TESTED + '.connection.get')
...
async def test_init(comms_mock, app, client):
...
async def test_on_data(comms_mock, sparky):
    assert len(sparky._requests) == 0

    await sparky._on_data(comms_mock, '05 00 |00 00 00 00')
    await asyncio.sleep(0.0001)
    assert len(sparky._requests) == 1
    assert sparky._requests['0500'].queue.qsize() == 1


async def test_on_data_error(mocker, comms_mock, sparky):
    logger_mock = mocker.spy(commander, 'LOGGER')

    # No response pipe
    await sparky._on_data(comms_mock, '1234')
    assert len(sparky._requests) == 0
    ...
    await sparky._on_data(comms_mock, 'BB|AA')
    assert len(sparky._requests) == 1
    assert logger_mock.error.call_count == 1


async def test_on_event(mocker, comms_mock, sparky):
    logger_mock = mocker.spy(commander, 'LOGGER')
    await sparky._on_event(comms_mock, 'hello')
    assert logger_mock.info.call_count == 1


async def test_on_welcome(app, mocker, comms_mock, sparky, welcome):
    ...
    await sparky._on_event(comms_mock, ok_msg)
    ...
    await sparky._on_event(comms_mock, nok_msg)
    assert service_status.desc(app).handshake_info.is_compatible_firmware
...
async def test_on_cbox_err(app, mocker, comms_mock, sparky, cbox_err):
    service_status.set_connected(app, 'addr')
    assert service_status.desc(app).device_address == 'addr'

    msg = ':'.join(cbox_err)
    await sparky._on_event(comms_mock, msg)
...
async def test_on_setup_mode(app, mocker, comms_mock, sparky):
    service_status.set_connected(app, 'addr')
    assert service_status.desc(app).device_address == 'addr'

    mocker.patch(TESTED + '.web.GracefulExit', RuntimeError)
    with pytest.raises(RuntimeError):
        await sparky._on_event(comms_mock, const.SETUP_MODE_PREFIX)


async def test_on_update(app, mocker, comms_mock, sparky):
    service_status.set_connected(app, 'addr')
    assert service_status.desc(app).device_address == 'addr'

    await sparky._on_event(comms_mock, const.UPDATER_PREFIX + '-message')


async def test_command(comms_mock, sparky, reset_msgid):
    await sparky._on_data(comms_mock, '01 00 07 28 | 00 00 00')
    ...
    comms_mock.write.assert_called_once_with('01000728')


async def test_error_command(comms_mock, sparky, reset_msgid):
    command = commands.ListStoredObjectsCommand.from_args()
    await sparky._on_data(comms_mock, '01 00 07 28 | FF 00 ')

    with pytest.raises(exceptions.CommandException):
        await sparky.execute(command)


async def test_timeout_command(comms_mock, sparky, mocker):
...
async def test_stale_reply(comms_mock, sparky, reset_msgid):
    # error code
...
async def test_queue_cleanup(mocker, comms_mock, sparky, app):
    ...
    await sparky._on_data(comms_mock, '05 00 |00 00 00')
...
async def test_start_update(mocker, comms_mock, sparky, app, reset_msgid):

    await sparky._on_data(comms_mock, '01 00 64 AF |00 00 00')
    await sparky.start_update(0.001)
    comms_mock.shutdown.assert_awaited()
