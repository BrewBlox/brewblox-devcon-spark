...
from .models import (
    ...
    StoredServiceSettingsBox,
    StoredServiceSettingsValue,
    StoredTimezoneSettingsBox,
    StoredTimezoneSettingsValue,
    StoredUnitSettingsBox,
...
class SettingsStore:
    ...
    async def _get_box(
        self, query: DatastoreSingleQuery, model: type[DatastoreSingleValueBox]
    ) -> DatastoreSingleValueBox:
    ...
    async def _set_box(self, box: DatastoreSingleValueBox):
    ...
    async def fetch_all(self):
        async with asyncio.timeout(self.config.datastore_fetch_timeout.total_seconds()):
            ...
            box = await self._get_box(
                query=DatastoreSingleQuery(id=self.config.name, namespace=const.SERVICE_NAMESPACE),
            ...
            # Fetch unit settings
            box = await self._get_box(
                query=DatastoreSingleQuery(id=const.GLOBAL_UNITS_ID, namespace=const.GLOBAL_NAMESPACE),
            ...
            box = await self._get_box(
                query=DatastoreSingleQuery(id=const.GLOBAL_TIME_ZONE_ID, namespace=const.GLOBAL_NAMESPACE),
