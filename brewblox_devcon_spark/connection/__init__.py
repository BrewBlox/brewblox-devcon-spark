...
async def discover(app: web.Application) -> BaseConnection:
...
async def connect(app: web.Application) -> BaseConnection:
    config: ServiceConfig = app['config']
    ...
    if mock:
        ...
        return await discover(app)


class ConnectionHandler(repeater.RepeaterFeature):
    def __init__(self, app: web.Application):
        ...
        self._data_callbacks: set[MessageCallback_] = set()
    ...
    async def run(self):
        ...
        try:
            ...
            self._connection = await connect(self.app)
            self._connection.on_event = self._on_event
            self._connection.on_response = self._on_data

            service_status.set_connected(self.app, self._connection.address)
            self._retry_count = 0
            ...
            with suppress(Exception):
                ...
                LOGGER.info(f'{self} closed connection')
