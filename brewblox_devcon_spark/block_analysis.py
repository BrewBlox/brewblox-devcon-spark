...
def calculate_limitations(blocks: List[dict]) -> List[dict]:
    """
    Finds all block constraints that are currently limiting.

    A constraint is limiting if it prevents desired state from becoming actual state.

    Args:
        blocks (List[dict]): Input block array.

    Returns:
        List[dict]: List of block limitations.
            Keys in dict:
            - target: str
            - constraint: str
    ...
    for block in blocks:
        try:
            # Check if block has constraints. We catch the KeyError.
            constraints = block['data']['constrainedBy']['constraints']

            # Iterate over constraints. Only yield active (limiting) ones
            for constraint in constraints:
                limiting = constraint.get('limiting')  # analog
                remaining = constraint.get('remaining')  # digital
                if limiting is False:
                    continue  # non-limiting analog constraint
                if remaining and not remaining.get('value'):
                    continue  # non-limiting digital constraint
                key = next((k for k
                            in constraint.keys()
                            if k not in ['remaining', 'limiting']
                            ), '??')  # pragma: no cover
                output.append({
                    'target': block['id'],
                    'constraint': key,
